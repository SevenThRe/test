...
public enum ShaderParameterFloat {
    ...
    public float eval(int index1, int index2) {
        ...
        switch (this) {
            case BIOME: {
                et pos = Shaders.getCameraPosition();
                anh biome = Shaders.getCurrentWorld().b(pos);
                return anh.a((anh)biome);
            }
            case TEMPERATURE: {
                et pos = Shaders.getCameraPosition();
                anh biome = Shaders.getCurrentWorld().b(pos);
                return biome != null ? biome.a(pos) : 0.0f;
            }
            case RAINFALL: {
                et pos = Shaders.getCameraPosition();
                anh biome = Shaders.getCurrentWorld().b(pos);
                return biome != null ? biome.k() : 0.0f;
            }
        }
