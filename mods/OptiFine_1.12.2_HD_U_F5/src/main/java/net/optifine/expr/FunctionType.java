...
public enum FunctionType {
    ...
    public float evalFloat(IExpression[] args) {
        switch (this) {
            ...
            case SIN: {
                return rk.a((float)FunctionType.evalFloat(args, 0));
            }
            case COS: {
                return rk.b((float)FunctionType.evalFloat(args, 0));
            }
            case ASIN: {
            ...
            case ATAN2: {
                return (float)rk.c((double)FunctionType.evalFloat(args, 0), (double)FunctionType.evalFloat(args, 1));
            }
            case TORAD: {
            ...
            case CLAMP: {
                return rk.a((float)FunctionType.evalFloat(args, 0), (float)FunctionType.evalFloat(args, 1), (float)FunctionType.evalFloat(args, 2));
            }
            case ABS: {
                return rk.e((float)FunctionType.evalFloat(args, 0));
            }
            case EXP: {
                return (float)Math.exp(FunctionType.evalFloat(args, 0));
            }
            case FLOOR: {
                return rk.d((float)FunctionType.evalFloat(args, 0));
            }
            case CEIL: {
                return rk.f((float)FunctionType.evalFloat(args, 0));
            }
            case FRAC: {
                return (float)rk.h((double)FunctionType.evalFloat(args, 0));
            }
            case LOG: {
            ...
            case SQRT: {
                return rk.c((float)FunctionType.evalFloat(args, 0));
            }
            case FMOD: {
                float fmodX = FunctionType.evalFloat(args, 0);
                float fmodY = FunctionType.evalFloat(args, 1);
                return fmodX - fmodY * (float)rk.d((float)(fmodX / fmodY));
            }
            case TIME: {
                bib mc = bib.z();
                bsb world = mc.f;
