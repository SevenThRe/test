 ...
 */
package org.yaml.snakeyamla.introspector;
...
import org.yaml.snakeyamla.introspector.FieldProperty;
import org.yaml.snakeyamla.introspector.MethodProperty;
import org.yaml.snakeyamla.introspector.MissingProperty;
import org.yaml.snakeyamla.introspector.Property;
import org.yaml.snakeyamla.util.PlatformFeatureDetector;
...
public class PropertyUtils {
    ...
    protected Map<String, Property> getPropertiesMap(Class<?> type, BeanAccess bAccess) {
        ...
        switch (bAccess) {
            ...
            default: {
                Object c2;
                try {
                    c2 = Introspector.getBeanInfo(type).getPropertyDescriptors();
                    int n2 = ((PropertyDescriptor[])c2).length;
                    for (int i2 = 0; i2 < n2; ++i2) {
                        PropertyDescriptor property = c2[i2];
                ...
                for (c2 = type; c2 != null; c2 = ((Class)c2).getSuperclass()) {
                    for (Field field : ((Class)c2).getDeclaredFields()) {
                        int modifiers = field.getModifiers();
    ...
    private boolean isTransient(FeatureDescriptor fd2) {
        return Boolean.TRUE.equals(fd2.getValue(TRANSIENT));
    }
