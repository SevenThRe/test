...
import org.yaml.snakeyamla.composer.ComposerException;
...
public class Composer {
    ...
    protected Node composeScalarNode(String anchor) {
        Tag nodeTag;
        ScalarEvent ev2 = (ScalarEvent)this.parser.getEvent();
        String tag = ev2.getTag();
        boolean resolved = false;
        if (tag == null || tag.equals("!")) {
            nodeTag = this.resolver.resolve(NodeId.scalar, ev2.getValue(), ev2.getImplicit().canOmitTagInPlainScalar());
            resolved = true;
        } else {
            nodeTag = new Tag(tag);
        }
        ScalarNode node = new ScalarNode(nodeTag, resolved, ev2.getValue(), ev2.getStartMark(), ev2.getEndMark(), ev2.getScalarStyle());
        if (anchor != null) {
