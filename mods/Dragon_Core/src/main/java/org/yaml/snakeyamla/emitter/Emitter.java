...
import org.yaml.snakeyamla.emitter.Emitable;
import org.yaml.snakeyamla.emitter.EmitterException;
import org.yaml.snakeyamla.emitter.EmitterState;
import org.yaml.snakeyamla.emitter.ScalarAnalysis;
import org.yaml.snakeyamla.error.YAMLException;
import org.yaml.snakeyamla.events.AliasEvent;
import org.yaml.snakeyamla.events.CollectionEndEvent;
import org.yaml.snakeyamla.events.CollectionStartEvent;
import org.yaml.snakeyamla.events.DocumentEndEvent;
import org.yaml.snakeyamla.events.DocumentStartEvent;
import org.yaml.snakeyamla.events.Event;
import org.yaml.snakeyamla.events.MappingEndEvent;
import org.yaml.snakeyamla.events.MappingStartEvent;
import org.yaml.snakeyamla.events.NodeEvent;
import org.yaml.snakeyamla.events.ScalarEvent;
import org.yaml.snakeyamla.events.SequenceEndEvent;
import org.yaml.snakeyamla.events.SequenceStartEvent;
import org.yaml.snakeyamla.events.StreamEndEvent;
import org.yaml.snakeyamla.events.StreamStartEvent;
import org.yaml.snakeyamla.reader.StreamReader;
import org.yaml.snakeyamla.scanner.Constant;
import org.yaml.snakeyamla.util.ArrayStack;
...
implements Emitable {
    ...
    private void processAnchor(String indicator) throws IOException {
        NodeEvent ev2 = (NodeEvent)this.event;
        if (ev2.getAnchor() == null) {
            this.preparedAnchor = null;
            return;
        }
        if (this.preparedAnchor == null) {
            this.preparedAnchor = Emitter.prepareAnchor(ev2.getAnchor());
        }
    ...
    private void processTag() throws IOException {
        String tag = null;
        if (this.event instanceof ScalarEvent) {
            ScalarEvent ev2 = (ScalarEvent)this.event;
            tag = ev2.getTag();
            if (this.style == null) {
                this.style = this.chooseScalarStyle();
            }
            if ((!this.canonical.booleanValue() || tag == null) && (this.style == null && ev2.getImplicit().canOmitTagInPlainScalar() || this.style != null && ev2.getImplicit().canOmitTagInNonPlainScalar())) {
                this.preparedTag = null;
                return;
            }
            if (ev2.getImplicit().canOmitTagInPlainScalar() && tag == null) {
                tag = "!";
                this.preparedTag = null;
            }
        } else {
            CollectionStartEvent ev3 = (CollectionStartEvent)this.event;
            tag = ev3.getTag();
            if ((!this.canonical.booleanValue() || tag == null) && ev3.getImplicit()) {
                this.preparedTag = null;
                return;
            }
        }
    ...
    private DumperOptions.ScalarStyle chooseScalarStyle() {
        ScalarEvent ev2 = (ScalarEvent)this.event;
        if (this.analysis == null) {
            this.analysis = this.analyzeScalar(ev2.getValue());
        }
        if (!ev2.isPlain() && ev2.getScalarStyle() == DumperOptions.ScalarStyle.DOUBLE_QUOTED || this.canonical.booleanValue()) {
            return DumperOptions.ScalarStyle.DOUBLE_QUOTED;
        }
        if (ev2.isPlain() && ev2.getImplicit().canOmitTagInPlainScalar() && (!this.simpleKeyContext || !this.analysis.isEmpty() && !this.analysis.isMultiline()) && (this.flowLevel != 0 && this.analysis.isAllowFlowPlain() || this.flowLevel == 0 && this.analysis.isAllowBlockPlain())) {
            return null;
        }
        if (!(ev2.isPlain() || ev2.getScalarStyle() != DumperOptions.ScalarStyle.LITERAL && ev2.getScalarStyle() != DumperOptions.ScalarStyle.FOLDED || this.flowLevel != 0 || this.simpleKeyContext || !this.analysis.isAllowBlock())) {
            return ev2.getScalarStyle();
        }
        if (!(!ev2.isPlain() && ev2.getScalarStyle() != DumperOptions.ScalarStyle.SINGLE_QUOTED || !this.analysis.isAllowSingleQuoted() || this.simpleKeyContext && this.analysis.isMultiline())) {
    ...
    private void processScalar() throws IOException {
        ...
        boolean bl2 = split = !this.simpleKeyContext && this.splitLines;
        if (this.style == null) {
    ...
    private void writeSingleQuoted(String text, boolean split) throws IOException {
        ...
        for (int end = 0; end <= text.length(); ++end) {
            ...
            if (spaces) {
                ...
                if (ch2 == '\u0000' || Constant.LINEBR.hasNo(ch2)) {
                    if (text.charAt(start) == '\n') {
                        this.writeLineBreak(null);
                    }
                    String data = text.substring(start, end);
                    for (char br2 : data.toCharArray()) {
                        if (br2 == '\n') {
                            this.writeLineBreak(null);
                            continue;
                        }
                        this.writeLineBreak(String.valueOf(br2));
                    }
                    this.writeIndent();
                    start = end;
                }
    ...
    private void writeDoubleQuoted(String text, boolean split) throws IOException {
        ...
        for (int end = 0; end <= text.length(); ++end) {
            ...
            if (ch2 == null || "\"\\\u0085\u2028\u2029\ufeff".indexOf(ch2.charValue()) != -1 || ' ' > ch2.charValue() || ch2.charValue() > '~') {
                ...
                if (ch2 != null) {
                    ...
                    if (ESCAPE_REPLACEMENTS.containsKey(ch2)) {
                        ...
                        String s2;
                        if (ch2.charValue() <= '\u00ff') {
                            s2 = "0" + Integer.toString(ch2.charValue(), 16);
                            data = "\\x" + s2.substring(s2.length() - 2);
                        } else if (ch2.charValue() >= '\ud800' && ch2.charValue() <= '\udbff') {
                            if (end + 1 < text.length()) {
                                Character ch22 = Character.valueOf(text.charAt(++end));
                                String s3 = "000" + Long.toHexString(Character.toCodePoint(ch2.charValue(), ch22.charValue()));
                                data = "\\U" + s3.substring(s3.length() - 8);
                            } else {
                                s2 = "000" + Integer.toString(ch2.charValue(), 16);
            ...
            if (0 >= end || end >= text.length() - 1 || ch2.charValue() != ' ' && start < end || this.column + (end - start) <= this.bestWidth || !split) continue;
    ...
    void writeFolded(String text, boolean split) throws IOException {
        ...
        for (int end = 0; end <= text.length(); ++end) {
            ...
            if (breaks) {
                if (ch2 == '\u0000' || Constant.LINEBR.hasNo(ch2)) {
                    if (!leadingSpace && ch2 != '\u0000' && ch2 != ' ' && text.charAt(start) == '\n') {
                        this.writeLineBreak(null);
                    }
                    leadingSpace = ch2 == ' ';
                    String data = text.substring(start, end);
    ...
    void writeLiteral(String text) throws IOException {
        ...
        for (int end = 0; end <= text.length(); ++end) {
            ...
            } else if (ch2 == '\u0000' || Constant.LINEBR.has(ch2)) {
    ...
    implements EmitterState {
        ...
        public void expect() throws IOException {
            if (Emitter.this.event instanceof DocumentStartEvent) {
                boolean implicit;
                DocumentStartEvent ev2 = (DocumentStartEvent)Emitter.this.event;
                if ((ev2.getVersion() != null || ev2.getTags() != null) && Emitter.this.openEnded) {
                    Emitter.this.writeIndicator("...", true, false, false);
                    Emitter.this.writeIndent();
                }
                if (ev2.getVersion() != null) {
                    String versionText = Emitter.this.prepareVersion(ev2.getVersion());
                    Emitter.this.writeVersionDirective(versionText);
                }
                Emitter.this.tagPrefixes = new LinkedHashMap(DEFAULT_TAG_PREFIXES);
                if (ev2.getTags() != null) {
                    TreeSet<String> handles = new TreeSet<String>(ev2.getTags().keySet());
                    for (String handle : handles) {
                        String prefix = ev2.getTags().get(handle);
                ...
                boolean bl2 = implicit = this.first && !ev2.getExplicit() && Emitter.this.canonical == false && ev2.getVersion() == null && (ev2.getTags() == null || ev2.getTags().isEmpty()) && !Emitter.this.checkEmptyDocument();
                if (!implicit) {
                    Emitter.this.writeIndent();
                    Emitter.this.writeIndicator("---", true, false, false);
                    if (Emitter.this.canonical.booleanValue()) {
