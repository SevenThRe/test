...
implements ImageTransformerCL {
    ...
    public ProjectiveColorTransformerCL(JavaCVCL context2, CvMat K1, CvMat K2, CvMat R, CvMat t2, CvMat n2, double[] referencePoints1, double[] referencePoints2, CvMat X, int numGains, int numBiases) {
        super(K1, K2, R, t2, n2, referencePoints1, referencePoints2, X, numGains, numBiases);
        ...
        if (this.getClass() == ProjectiveColorTransformerCL.class) {
            CLKernel[] kernels = context2.buildKernels("-cl-fast-relaxed-math -cl-mad-enable -DDOT_SIZE=" + dotSize, "ImageTransformer.cl:ProjectiveColorTransformer.cl", "transformOne", "transformSub", "transformDot", "reduceOutputData");
    ...
    protected void prepareHomographies(CLBuffer HBuffer, int pyramidLevel, ImageTransformer.Parameters[] parameters, boolean[] inverses) {
        ...
        for (int i2 = 0; i2 < parameters.length; ++i2) {
            this.prepareHomography(H, pyramidLevel, (ProjectiveColorTransformer.Parameters)parameters[i2], inverses == null ? false : inverses[i2]);
    ...
    protected void prepareColorTransforms(CLBuffer XBuffer, int pyramidLevel, ImageTransformer.Parameters[] parameters, boolean[] inverses) {
        ...
        for (int i2 = 0; i2 < parameters.length; ++i2) {
            this.prepareColorTransform(X2, pyramidLevel, (ProjectiveColorTransformer.Parameters)parameters[i2], inverses == null ? false : inverses[i2]);
            for (int j2 = 0; j2 < 16; ++j2) {
                floatX.put((float)X2.get(j2));
            }
        }
        floatX.rewind();
    }
