...
extends AnnotatedMixinElementHandler {
    ...
    private boolean registerInjector(AnnotatedElementInjector elem, String reference, ITargetSelectorRemappable targetMember, TypeHandle target) {
        ...
        try {
            ...
            if (elem.hasCoerceArgument() && targetMember.getOwner() == null && targetMember.getDesc() == null) {
                String newName;
                ITargetSelector oldMember = TargetSelector.parse(ex2.getOld());
                ITargetSelector newMember = TargetSelector.parse(ex2.getNew());
    ...
    protected final void remapReference(String subject, String reference, AnnotatedElementInjectionPoint elem) {
        ...
        try {
            ...
            this.ap.printMessage(Diagnostic.Kind.ERROR, (CharSequence)ex2.getMessage(), (Element)elem.getElement(), errorsOn);
        }
        try {
            ...
            this.ap.printMessage(Diagnostic.Kind.ERROR, (CharSequence)("Unexpected reference conflict for " + subject + ": " + reference + " -> " + ex3.getNew() + " previously defined as " + ex3.getOld()), (Element)elem.getElement(), errorsOn);
            return;
        }
        elem.notifyRemapped();
    }
