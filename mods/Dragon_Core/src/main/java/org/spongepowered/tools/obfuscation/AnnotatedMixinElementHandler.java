...
abstract class AnnotatedMixinElementHandler {
    ...
    protected final void checkConstraints(ExecutableElement method, AnnotationHandle annotation) {
        try {
            ...
            try {
                ...
                this.ap.printMessage(Diagnostic.Kind.ERROR, (CharSequence)ex2.getMessage(), (Element)method, annotation.asMirror());
            }
        }
        catch (InvalidConstraintException ex3) {
            this.ap.printMessage(Diagnostic.Kind.WARNING, (CharSequence)ex3.getMessage(), (Element)method, annotation.asMirror(), SuppressedBy.CONSTRAINTS);
        }
    }
    ...
    protected final void validateTargetMethod(ExecutableElement method, AnnotationHandle annotation, AliasedElementName name, String type, boolean overwrite, boolean merge) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            ...
            if (merge) continue;
            this.printMessage(Diagnostic.Kind.WARNING, "Cannot find target for " + type + " method in " + target, method, annotation, SuppressedBy.TARGET);
        }
    }
    ...
    private void validateMethodVisibility(ExecutableElement method, AnnotationHandle annotation, String type, TypeHandle target, MethodHandle targetMethod) {
        ...
        if (visTarget.ordinal() > visMethod.ordinal()) {
            this.printMessage(Diagnostic.Kind.WARNING, (Object)((Object)visMethod) + " " + type + " method cannot reduce " + visibility, method, annotation, SuppressedBy.VISIBILITY);
        } else if (visTarget == Visibility.PRIVATE && visMethod.ordinal() > visTarget.ordinal()) {
            this.printMessage(Diagnostic.Kind.WARNING, (Object)((Object)visMethod) + " " + type + " method will upgrade " + visibility, method, annotation, SuppressedBy.VISIBILITY);
        }
    }
    ...
    protected final void validateTargetField(VariableElement field, AnnotationHandle annotation, AliasedElementName name, String type) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            ...
            this.ap.printMessage(Diagnostic.Kind.WARNING, (CharSequence)("Cannot find target for " + type + " field in " + target), (Element)field, annotation.asMirror(), SuppressedBy.TARGET);
        }
    }
    ...
    protected final void validateReferencedTarget(ExecutableElement method, AnnotationHandle inject, ITargetSelector reference, String type) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            ...
            this.ap.printMessage(Diagnostic.Kind.WARNING, (CharSequence)("Cannot find target method for " + type + " in " + target), (Element)method, inject.asMirror(), SuppressedBy.TARGET);
        }
    }

    private void printMessage(Diagnostic.Kind kind, String msg, Element e2, AnnotationHandle annotation, SuppressedBy suppressedBy) {
        if (annotation == null) {
            this.ap.printMessage(kind, (CharSequence)msg, e2, suppressedBy);
        } else {
            this.ap.printMessage(kind, (CharSequence)msg, e2, annotation.asMirror(), suppressedBy);
        }
    }
    ...
    static abstract class AnnotatedElement<E extends Element> {
        ...
        public final void printMessage(IMessagerSuppressible messager, Diagnostic.Kind kind, CharSequence msg, SuppressedBy suppressedBy) {
            messager.printMessage(kind, msg, (Element)this.element, this.annotation.asMirror(), suppressedBy);
        }
    }
}
