...
extends AnnotatedMixinElementHandler {
    ...
    public void process(AnnotationHandle implementsAnnotation) {
        ...
        if (interfaces.size() < 1) {
            this.ap.printMessage(Diagnostic.Kind.WARNING, (CharSequence)"Empty @Implements annotation", (Element)this.mixin.getMixin(), implementsAnnotation.asMirror());
            return;
        }
        for (AnnotationHandle interfaceAnnotation : interfaces) {
            ...
            try {
                ...
                this.ap.printMessage(Diagnostic.Kind.ERROR, (CharSequence)("Unexpected error: " + ex2.getClass().getName() + ": " + ex2.getMessage()), (Element)this.mixin.getMixin(), interfaceAnnotation.asMirror());
            }
        }
    }
