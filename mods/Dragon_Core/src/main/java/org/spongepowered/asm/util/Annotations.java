...
public final class Annotations {
    ...
    public static void merge(ClassNode from, ClassNode to2) {
        to2.visibleAnnotations = Annotations.merge(from.visibleAnnotations, to2.visibleAnnotations, "class", from.name);
        to2.invisibleAnnotations = Annotations.merge(from.invisibleAnnotations, to2.invisibleAnnotations, "class", from.name);
    }

    public static void merge(MethodNode from, MethodNode to2) {
        to2.visibleAnnotations = Annotations.merge(from.visibleAnnotations, to2.visibleAnnotations, "method", from.name);
        to2.invisibleAnnotations = Annotations.merge(from.invisibleAnnotations, to2.invisibleAnnotations, "method", from.name);
    }

    public static void merge(FieldNode from, FieldNode to2) {
        to2.visibleAnnotations = Annotations.merge(from.visibleAnnotations, to2.visibleAnnotations, "field", from.name);
        to2.invisibleAnnotations = Annotations.merge(from.invisibleAnnotations, to2.invisibleAnnotations, "field", from.name);
    }

    private static List<AnnotationNode> merge(List<AnnotationNode> from, List<AnnotationNode> to2, String type, String name) {
        try {
            if (from == null) {
                return to2;
            }
            if (to2 == null) {
                to2 = new ArrayList<AnnotationNode>();
            }
            for (AnnotationNode annotation : from) {
                if (!Annotations.isMergeableAnnotation(annotation)) continue;
                Iterator<AnnotationNode> iter = to2.iterator();
                while (iter.hasNext()) {
    ...
    private static Pattern getMergeableAnnotationPattern() {
        ...
        for (int i2 = 0; i2 < MERGEABLE_MIXIN_ANNOTATIONS.length; ++i2) {
            ...
            sb2.append(MERGEABLE_MIXIN_ANNOTATIONS[i2].getName().replace('.', '/'));
        }
        return Pattern.compile(sb2.append(");$").toString());
    }
