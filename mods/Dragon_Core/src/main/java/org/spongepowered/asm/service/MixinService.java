    ...
    private void runBootServices() {
        ...
        while (iter.hasNext()) {
            try {
                ...
                logger.debug("Mixin bootstrap service {} is not available: {}", new Object[]{ex2.getStackTrace()[0].getClassName(), ex2.getMessage()});
            }
            catch (Throwable th2) {
                logger.debug("Catching {}:{} initialising service", new Object[]{th2.getClass().getName(), th2.getMessage(), th2});
            }
        }
    }
    ...
    private IMixinService initService() {
        ...
        while (iter.hasNext()) {
            try {
                ...
                logger.debug("MixinService [{}] failed initialisation: {}", new Object[]{faultingClassName, th2.getMessage()});
