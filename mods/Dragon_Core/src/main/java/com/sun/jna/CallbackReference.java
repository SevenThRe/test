...
extends WeakReference<Callback> {
    ...
    private static Callback getCallback(Class<?> type, Pointer p2, boolean direct) {
        ...
        synchronized (map2) {
            ...
            cb2 = (Callback)Proxy.newProxyInstance(type.getClassLoader(), new Class[]{type}, (InvocationHandler)h2);
            map.remove(cb2);
        ...
        private Object invokeCallback(Object[] args) {
            ...
            for (int i2 = 0; i2 < args.length; ++i2) {
                ...
                if (this.fromNative[i2] != null) {
                    CallbackParameterContext context2 = new CallbackParameterContext(type, this.callbackMethod, args, i2);
                    callbackArgs[i2] = this.fromNative[i2].fromNative(arg, context2);
                    continue;
                }
