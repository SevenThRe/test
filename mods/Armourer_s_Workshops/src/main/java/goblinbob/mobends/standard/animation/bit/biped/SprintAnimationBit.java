...
extends AnimationBit<T> {
    ...
    public void perform(T data) {
        ...
        ((BipedEntityData)data).leftForeLeg.rotation.setSmoothness(0.7f).orientX(40.0f + MathHelper.cos((float)(limbSwing + 1.8f)) * foreLegSwingAmount);
        ((BipedEntityData)data).rightForeLeg.rotation.setSmoothness(0.7f).orientX(40.0f + MathHelper.cos((float)(limbSwing + (float)Math.PI + 1.8f)) * foreLegSwingAmount);
        ...
        float bodyRotationY = MathHelper.cos((float)limbSwing) * -40.0f;
        float bodyRotationX = MathHelper.cos((float)(limbSwing * 2.0f)) * 10.0f + 10.0f;
        ...
        ((BipedEntityData)data).globalOffset.slideY(MathHelper.cos((float)(limbSwing * 2.0f + 0.6f)) * 1.5f, 0.9f);
    }
}
