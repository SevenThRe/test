...
extends AnnotatedMixinElementHandler {
    ...
    public void registerOverwrite(AnnotatedElementOverwrite elem) {
        ...
        if (!"true".equalsIgnoreCase(this.ap.getOption("disableOverwriteChecker"))) {
            ...
            if (javadoc == null) {
                this.ap.printMessage(overwriteErrorKind, (CharSequence)"@Overwrite is missing javadoc comment", (Element)elem.getElement(), SuppressedBy.OVERWRITE);
                return;
            }
            if (!javadoc.toLowerCase(Locale.ROOT).contains("@author")) {
                this.ap.printMessage(overwriteErrorKind, (CharSequence)"@Overwrite is missing an @author tag", (Element)elem.getElement(), SuppressedBy.OVERWRITE);
            }
            if (!javadoc.toLowerCase(Locale.ROOT).contains("@reason")) {
                this.ap.printMessage(overwriteErrorKind, (CharSequence)"@Overwrite is missing an @reason tag", (Element)elem.getElement(), SuppressedBy.OVERWRITE);
            }
        }
    }
