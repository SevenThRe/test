...
extends MixinException {
    ...
    private PrettyPrinter printProblem(PrettyPrinter printer, IMixinContext context, MethodNode mda, MethodNode mdb) {
        Type target = Type.getObjectType((String)context.getTargetClassRef());
        ...
        switch (this.problem) {
            ...
            case BAD_LOAD: {
                ...
                ListIterator ia2 = mda.instructions.iterator();
                ListIterator ib2 = mdb.instructions.iterator();
                Type[] argsa = Type.getArgumentTypes((String)mda.desc);
                Type[] argsb = Type.getArgumentTypes((String)mdb.desc);
                int index = 0;
                while (ia2.hasNext() && ib2.hasNext()) {
                    AbstractInsnNode na2 = (AbstractInsnNode)ia2.next();
                    AbstractInsnNode nb2 = (AbstractInsnNode)ib2.next();
                    if (na2 instanceof VarInsnNode && nb2 instanceof VarInsnNode) {
                        ...
                        Type tb2 = vb2.var > 0 ? argsb[vb2.var - 1] : target;
                        printer.kv("Target " + index, "%8s %-2d %s", Bytecode.getOpcodeName((AbstractInsnNode)va2), va2.var, ta2);
                        printer.kv("Incoming " + index, "%8s %-2d %s", Bytecode.getOpcodeName((AbstractInsnNode)vb2), vb2.var, tb2);
                        if (ta2.equals((Object)tb2)) {
                            printer.kv("", "Types match: %s", ta2);
                        } else if (ta2.getSort() != tb2.getSort()) {
                            printer.kv("", "Types are incompatible");
                        } else if (ta2.getSort() == 10) {
                            ClassInfo superClass = ClassInfo.getCommonSuperClassOrInterface(ta2, tb2);
            ...
            case BAD_CAST: {
                ...
                Type ta3 = Type.getObjectType((String)((TypeInsnNode)this.a).desc);
                Type tb3 = Type.getObjectType((String)((TypeInsnNode)this.b).desc);
                printer.kv("Target type", ta3);
                printer.kv("Incoming type", tb3);
                printer.kv("Common supertype", ClassInfo.getCommonSuperClassOrInterface(ta3, tb3)).add();
                break;
            }
            ...
            case BAD_INVOKE_DESC: {
                ...
                Type[] arga = Type.getArgumentTypes((String)mdna.desc);
                Type[] argb = Type.getArgumentTypes((String)mdnb.desc);
                if (arga.length != argb.length) {
                    int argCount = Type.getArgumentTypes((String)mda.desc).length;
                ...
                Type rta = Type.getReturnType((String)mdna.desc);
                Type rtb = Type.getReturnType((String)mdnb.desc);
