...
extends Thread {
    ...
    private static Map<String, File> groups = new ConcurrentHashMap<String, File>();
    ...
    public void run() {
        try {
            final Collection<File> values = groups.values();
            Files.walkFileTree(CACHE_DIR.toPath(), (FileVisitor<? super Path>)new FileVisitor<Path>(){

                @Override
                public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) throws IOException {
                    for (Path usedFile : usedFiles) {
                        if (!usedFile.equals(dir)) continue;
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                    for (Path usedFile : usedFiles) {
                        if (!usedFile.equals(file)) continue;
                        return FileVisitResult.CONTINUE;
                    }
                    Files.delete(file);
                    return FileVisitResult.CONTINUE;
            ...
            AncientDream.getLogger().error("Error while delete unused cache", (Throwable)e);
        }
    }
