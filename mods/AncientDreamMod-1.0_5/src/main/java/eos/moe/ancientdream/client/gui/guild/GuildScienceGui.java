...
extends BaseGui {
    ...
    public void func_73866_w_() {
        super.func_73866_w_();
        double scale = this.field_146295_m < 400 ? 2.0 : 1.0;
        int maxX = 0;
        for (ScienceData science : this.data.sciences) {
            maxX = Math.max(maxX, science.x);
        }
        this.maxScrollDistance = (float)Math.max(-((double)this.field_146294_l - (double)(maxX + 180) / scale), 0.0);
    }
    ...
    public void func_73863_a(int mouseX, int mouseY, float partialTicks) {
        ...
        GlStateManager.func_179137_b((double)0.0, (double)(((double)this.field_146295_m - height) / 2.0), (double)0.0);
        RenderUtils.drawTexture(0.0, 0.0, subWidth, height, new ResourceLocation("ancientdream", "gui/guild/science1.png"));
        RenderUtils.drawTexture(subWidth, 0.0, (double)(this.scrollDistance * this.maxScrollDistance), 0.0, (double)this.field_146294_l - subWidth * 2.0, height, bgw, height, BG);
        RenderUtils.drawTexture((double)this.field_146294_l - subWidth, 0.0, subWidth, height, new ResourceLocation("ancientdream", "gui/guild/science2.png"));
        this.func_73730_a((int)(subWidth - 4.0), (int)((double)this.field_146294_l - subWidth + 4.0), (int)(height + 9.0), -16777216);
        RenderUtils.drawTexture(subWidth + (double)this.scrollDistance * ((double)(this.field_146294_l - 37) - subWidth * 2.0), height + 1.0, 37.0, 16.5, new ResourceLocation("ancientdream", "gui/guild/sciencebar.png"));
        GL11.glEnable((int)3089);
        RenderUtils.scissorBox((int)subWidth, 0.0f, (int)((double)this.field_146294_l - 2.0 * subWidth), this.field_146295_m);
        for (ScienceData science : this.data.sciences) {
            ...
            if (!Utils.onArea(x, ((double)this.field_146295_m - height) / 2.0 + y, 150.0 / scale, 42.0 / scale, (double)mouseX, (double)mouseY)) continue;
            this.hoverScience = science;
        }
        GL11.glDisable((int)3089);
        GlStateManager.func_179121_F();
        RenderUtils.drawText("Tip: \u53cc\u51fb\u79d1\u6280\u5373\u53ef\u8fdb\u884c\u7814\u7a76!  \u5f53\u524d\u516c\u4f1a\u8d44\u91d1: \u91d1\u5e01x" + this.data.money + ",\u70b9\u5238x" + this.data.points + ",\u8d21\u732ex" + this.data.score, 5.0, this.field_146295_m - 12, false, true);
        if (this.hoverScience != null) {
            this.func_146283_a(this.hoverScience.tip, mouseX, mouseY);
        }
        if (this.scrollPoint != null) {
            int x = mouseX - this.scrollPoint.x;
            this.scrollDistance = (float)((double)x / ((double)(this.field_146294_l - 37) - subWidth * 2.0));
    ...
    protected void func_73864_a(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        double scale = this.field_146295_m < 400 ? 2.0 : 1.0;
        double subWidth = 20.0 / scale;
        if (Utils.onArea(20.0 / scale + (double)this.scrollDistance * ((double)(this.field_146294_l - 37) - subWidth * 2.0), ((double)this.field_146295_m - 400.0 / scale) / 2.0 + 400.0 / scale + 1.0, 37.0, 16.5, (double)mouseX, (double)mouseY)) {
            this.scrollPoint = new Point((int)((double)mouseX - (double)this.scrollDistance * ((double)(this.field_146294_l - 37) - subWidth * 2.0)), mouseY);
        }
    }
    ...
    static {
        try {
            IResource resource = Minecraft.func_71410_x().func_110442_L().func_110536_a(BG);
