...
public class GuiCrosshairsBCRenderAttackIndicator
extends ShoulderTransformer {
    @Override
    protected InsnList searchList(Mappings mappings, boolean obf) {
        InsnList searchList = new InsnList();
        searchList.add((AbstractInsnNode)new FieldInsnNode(180, mappings.map("GameSettings", false), mappings.map("GameSettings#thirdPersonViewSRG", obf), mappings.desc("GameSettings#thirdPersonViewSRG", obf)));
        return searchList;
    }
