...
implements Comparable<TopoProperties> {
    ...
    public Integer[] getLandColors() {
        if (this.landColors == null) {
            ...
            if (this.landColors == null || this.landColors.length == 0) {
                ...
                this.landColors = this.getColors("land", DEFAULT_LAND_COLORS);
            }
        }
    ...
    public Integer[] getWaterColors() {
        if (this.waterColors == null) {
            ...
            if (this.waterColors == null || this.waterColors.length == 0) {
                ...
                this.waterColors = this.getColors("water", DEFAULT_WATER_COLORS);
            }
        }
    ...
    public Integer getLandContourColor() {
        if (!this.showContour.get().booleanValue()) {
            return null;
        }
    ...
    public boolean isValid(boolean fix) {
        ...
        valid = this.isValid(this.land.get(), fix) && valid;
        return valid;
    }
    ...
    private boolean isValid(String colorString, boolean fix) {
        ...
        for (int i = 0; i < colors.length; ++i) {
            ...
            if (Objects.equal((Object)colorStr, (Object)colors[i])) continue;
            if (fix) {
                colors[i] = colorStr;
                continue;
            }
