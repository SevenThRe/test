...
implements Dumpable {
    ...
    private Runnable processConnect(SelectionKey key, final Connect connect2) {
        SelectableChannel channel = key.channel();
        try {
            key.attach(connect2.attachment);
            boolean connected = this._selectorManager.doFinishConnect(channel);
            if (LOG.isDebugEnabled()) {
                LOG.debug("Connected {} {}", connected, channel);
            }
            if (connected) {
                if (connect2.timeout.cancel()) {
                    key.interestOps(0);
    ...
    extends NonBlockingAction {
        ...
        private ConnectTimeout(Connect connect2) {
