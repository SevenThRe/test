...
implements BlockingQueue<E> {
    ...
    public E poll() {
        ...
        try {
            if (this._size.get() > 0) {
                int head2 = this._indexes[HEAD_OFFSET];
                e = this._elements[head2];
                this._elements[head2] = null;
                this._indexes[BlockingArrayQueue.HEAD_OFFSET] = (head2 + 1) % this._elements.length;
        ...
        try {
            ...
            try {
                ...
                if (this.size() > 0) {
                    ...
                    int tail = this._indexes[TAIL_OFFSET];
                    if (head2 < tail) {
                        System.arraycopy(this._elements, head2, elements, 0, tail - head2);
                    } else {
                        int chunk = this._elements.length - head2;
                        System.arraycopy(this._elements, head2, elements, 0, chunk);
    ...
    private boolean grow() {
        ...
        try {
            ...
            try {
                ...
                if (head2 < tail) {
                    newTail = tail - head2;
                    System.arraycopy(this._elements, head2, elements, 0, newTail);
                } else if (head2 > tail || this._size.get() > 0) {
                    newTail = capacity + tail - head2;
                    int cut = capacity - head2;
                    System.arraycopy(this._elements, head2, elements, 0, cut);
