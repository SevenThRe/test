...
Connection.UpgradeFrom {
    ...
    @Override
    public void send(MetaData.Response info, boolean head2, ByteBuffer content, boolean lastContent, Callback callback) {
        if (info == null) {
        ...
        if (this._sendCallback.reset(info, head2, content, lastContent, callback)) {
    ...
    extends IteratingCallback {
        ...
        private boolean reset(MetaData.Response info, boolean head2, ByteBuffer content, boolean last, Callback callback) {
            if (this.reset()) {
                this._info = info;
                this._head = head2;
