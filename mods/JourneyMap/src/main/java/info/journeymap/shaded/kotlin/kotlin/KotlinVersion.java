...
implements Comparable<KotlinVersion> {
    ...
    private final int versionOf(int major, int minor, int patch2) {
        int n = major;
        n = 0 <= n && 255 >= n && 0 <= (n = minor) && 255 >= n && 0 <= (n = patch2) && 255 >= n ? 1 : 0;
        boolean bl = false;
        boolean bl2 = false;
        if (n == 0) {
            boolean bl3 = false;
            String string = "Version components are out of range: " + major + '.' + minor + '.' + patch2;
            throw (Throwable)new IllegalArgumentException(string.toString());
        }
        return (major << 16) + (minor << 8) + patch2;
    }
    ...
    public final boolean isAtLeast(int major, int minor, int patch2) {
        return this.major > major || this.major == major && (this.minor > minor || this.minor == minor && this.patch >= patch2);
    }
    ...
    public KotlinVersion(int major, int minor, int patch2) {
        this.major = major;
        this.minor = minor;
        this.patch = patch2;
