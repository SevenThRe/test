...
public class Deinterlacer {
    ...
    void setPass(int p) {
        ...
        this.pass = p;
        byte[] pp = Deinterlacer.paramsForPass(p);
        this.dX = pp[0];
        this.dY = pp[1];
        this.oX = pp[2];
        this.oY = pp[3];
        this.rows = this.imi.rows > this.oY ? (this.imi.rows + this.dY - 1 - this.oY) / this.dY : 0;
        int n = this.cols = this.imi.cols > this.oX ? (this.imi.cols + this.dX - 1 - this.oX) / this.dX : 0;
        if (this.cols == 0) {
            this.rows = 0;
        }
    ...
    public int getTotalRows() {
        if (this.totalRows == 0) {
            for (int p = 1; p <= 7; ++p) {
                ...
                int n = cols = this.imi.cols > pp[2] ? (this.imi.cols + pp[0] - 1 - pp[2]) / pp[0] : 0;
