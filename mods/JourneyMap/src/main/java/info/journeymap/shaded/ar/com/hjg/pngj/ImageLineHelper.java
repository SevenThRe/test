...
public class ImageLineHelper {
    ...
    static int[] lineToARGB32(ImageLineByte line, PngChunkPLTE pal, PngChunkTRNS trns, int[] buf) {
        ...
        if (line.getImageInfo().indexed) {
            ...
            for (int c = 0; c < cols; ++c) {
                int index = line.scanline[c] & 0xFF;
            ...
            int c2 = 0;
            for (int c = 0; c < cols; ++c) {
                int g = line.scanline[c2++] & 0xFF;
                int alpha = alphachannel ? line.scanline[c2++] & 0xFF : (g != ga ? 255 : 0);
            ...
            int c2 = 0;
            for (int c = 0; c < cols; ++c) {
                int rgb = (line.scanline[c2++] & 0xFF) << 16 | (line.scanline[c2++] & 0xFF) << 8 | line.scanline[c2++] & 0xFF;
                int alpha = alphachannel ? line.scanline[c2++] & 0xFF : (rgb != ga ? 255 : 0);
                buf[c] = alpha << 24 | rgb;
            }
        }
        return buf;
    ...
    static byte[] lineToRGBA8888(ImageLineByte line, PngChunkPLTE pal, PngChunkTRNS trns, byte[] buf) {
        ...
        if (line.imgInfo.indexed) {
            ...
            while (b < bytes) {
                ...
                buf[b++] = val;
                buf[b++] = val;
                buf[b++] = alphachannel ? line.scanline[c++] : ((val & 0xFF) == ga ? 0 : -1);
            }
            ...
            while (b < bytes) {
                ...
                buf[b++] = line.scanline[c++];
                buf[b++] = -1;
                if (trns == null || buf[b - 3] != (byte)trns.getRGB()[0] || buf[b - 2] != (byte)trns.getRGB()[1] || buf[b - 1] != (byte)trns.getRGB()[2]) continue;
                buf[b - 1] = 0;
            }
        }
        return buf;
    ...
    public static double int2doubleClamped(ImageLineInt line, int p) {
        double d;
        double d2 = d = line.imgInfo.bitDepth == 16 ? (double)p / 65535.0 : (double)p / 255.0;
